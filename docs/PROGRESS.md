...
- **Domain-layer breaking change to `IDomainEvent`.** The current default-implemented `EventId` / `OccurredOn` getters return fresh values on every read, which is unsuitable for outbox round-trips. M8 makes both members init-set with default initializers (values captured at
construction time, immutable thereafter, round-trippable through `System.Text.Json`)
and renames `OccurredOn` to `OccurredOnUtc`. The six existing events (`AppointmentScheduledEvent`, `AppointmentCancelledEvent`, `AppointmentCompletedEvent`, `VaccinationRecordedEvent`, `OwnerCreatedEvent`, `PetRegisteredEvent`) absorb the change in PR (1).
- **New EF migration target `AddOutboxMessages`.** Creates `vetcare.outbox_messages` (`Id`, `OccurredOnUtc`, `Type`, `Content (jsonb)`, `TenantId`, `ProcessedOnUtc`, `Error`, `Attempts`) with a `(ProcessedOnUtc, OccurredOnUtc)` index for the worker's next-batch scan. A follow-up migration `AddOutboxMessageNextAttemptOnUtc` (landed in PR (3)) adds the nullable `NextAttemptOnUtc (timestamptz)` column so the worker's exponential-backoff state survives process restarts. Backout is purely additive — drop both migrations, remove the worker, restore the synchronous publish.
...
- **Test strategy.** Domain unit tests verify per-event metadata stability across reads (proving the init-set default initializers persist values) and per-event `System.Text.Json` round-trip equality. Testcontainers integration tests cover the `SaveChangesAsync` write path (happy + no-events + multi-event), the worker's full lifecycle (failing-publisher → outbox row + `Error` + `Attempts == 1` + `NextAttemptOnUtc` in the future; recovery after substitute reset and `NextAttemptOnUtc` elapses → `ProcessedOnUtc` set + `Error` cleared + `Attempts` unchanged; backoff skip while `NextAttemptOnUtc` is in the future; exclusion of pre-poisoned rows already at `MaxAttempts`), and the `CurrentTenantProvider.SetTenant` override. Newman E2E exercises the worker indirectly through the existing
`schedule → confirm → vaccination` flow; assertions stay HTTP-side and do not
directly observe SQS consumption.
...
- **Delivery summary.** Four PRs landed back-to-back:
  ...
  Domain 39 + Application 36 + Integration 52 (50 at PR (4) cutover; +1 from
`fix/m8-outbox-tenant-capture` and +1 from `test/m8-outbox-recovery` during
post-delivery code-review fixes), lint pass. ADR-009
...
- **`ISqsPublisher` and `IStorageService` stay as NSubstitute fakes.** Bringing LocalStack into Testcontainers would require additional bootstrap (queue creation, S3 bucket creation, region/credentials wiring) that does not pay off for the current assertions; tracked in `docs/BACKLOG.md`. The SQS publish path is exercised indirectly by the Newman E2E job (the
`Schedule appointment` step calls the API, which publishes via the real
`ISqsPublisher` against the LocalStack queue), but Newman does not assert
SQS consumption. S3 round-trips are not currently covered — the pet-photo
upload endpoint is not part of the Newman collection; tracked in
`docs/BACKLOG.md`.
